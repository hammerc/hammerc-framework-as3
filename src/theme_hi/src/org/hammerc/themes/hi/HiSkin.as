...
package org.hammerc.themes.hi
{
	...
	/**
	 * <code>HiSkin</code> 类定义了 Hi 主题的皮肤基类.
	 * @author wizardc
	 */
	public class HiSkin extends SkinBase
	{
		 ...
		 * 文本投影滤镜.
		 */
		hammerc_internal static var _textOverFilter:Array = [new DropShadowFilter(1, 270, 0, 0.3, 1, 1)];
		
		/**
		 * 边框线条颜色.
		 */
		hammerc_internal static var _borderColors:Array = [0xD4D4D4, 0x518CC6, 0x686868];
		
		/**
		 * 底线颜色.
		 */
		hammerc_internal static var _bottomLineColors:Array = [0xBCBCBC, 0x2A65A0, 0x656565];
		
		/**
		 * 边框圆角.
		 */
		hammerc_internal static var _cornerRadius:Number = 3;
		
		/**
		 * 填充颜色 .
		 */
		hammerc_internal static var _fillColors:Array = [0xFAFAFA, 0xEAEAEA, 0x589ADB, 0x3173B4, 0x777777, 0x9B9B9B];
		...
		{
			_tempMatrix.createGradientBox(width, height, 0.5 * Math.PI, x, y);
			return _tempMatrix;
		}
		...
		{
			_tempMatrix.createGradientBox(width, height, 0, x, y);
			return _tempMatrix;
		}
		 ...
		 * @param w 矩形的宽度 (以像素为单位).
		 * @param h 矩形的高度 (以像素为单位).
		 * @param borderColor 外框线条颜色.
		 * @param bottomLineColor 底线颜色.
		 * @param fillColors 填充色.
		 * @param cornerRadius 矩形的角半径.
		 * @param g 要绘制到的目标 Graphics, 留空则绘制到自身.
		 */
		hammerc_internal function drawCurrentState(x:Number, y:Number, w:Number, h:Number, borderColor:uint, bottomLineColor:uint, fillColors:Object, cornerRadius:Object = null, g:Graphics = null):void
		{
			...
			if(cornerRadius == null || cornerRadius is Number)
			{
				...
				crr1 = {tl:crr1, tr:crr1, bl:crr1, br:crr1};
			}
			else
			{
				crr1 = {tl : Math.max(0, crr.tl - 1), tr : Math.max(0, crr.tr - 1), bl : Math.max(0, crr.bl - 1), br : Math.max(0, crr.br - 1)};
			}
			//绘制边框
			this.drawRoundRect(x, y, w, h, cornerRadius, borderColor, 1, this.verticalGradientMatrix(x, y, w, h), GradientType.LINEAR, null, {x:x + 1, y:y + 1, w:w - 2, h:h - 2, r:crr1}, g); 
			//绘制填充
			this.drawRoundRect(x + 1, y + 1, w - 2, h - 2, crr1, fillColors, 1, this.verticalGradientMatrix(x + 1, y + 1, w - 2, h - 2), GradientType.LINEAR, null, null, g);
			//绘制底线
			if(w > (crr1.bl + crr1.br + 2) && h > 1)
			{
				this.drawLine(x + crr1.bl, y + h - 1, x + w - crr1.br, y + h - 1, bottomLineColor, g);
			}
		}
		
		/**
		 * 绘制一条直线.
		 * @param startX 起始点坐标 x.
		 * @param startY 起始点坐标 y.
		 * @param endX 结束点坐标 x.
		 * @param endY 结束点坐标 y.
		 * @param color 线条颜色.
		 * @param g 要绘制到的目标 Graphics, 留空则绘制到自身.
		 ...
		 */
		protected function drawRoundRect(x:Number, y:Number, width:Number, height:Number, cornerRadius:Object = null, color:Object = null, alpha:Object = null, gradientMatrix:Matrix = null, gradientType:String = "linear", gradientRatios:Array = null, hole:Object = null, g:Graphics = null):void
		{
			if(g == null)
			{
				g = graphics;
